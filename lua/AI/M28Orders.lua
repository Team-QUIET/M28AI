...
function IssueTrackedClearCommands(oUnit)
    ...
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        ...
        if EntityCategoryContains(M28UnitInfo.refCategoryEngineer + categories.COMMAND + categories.SUBCOMMANDER, oUnit.UnitId) then
            --Dont clear active shield engineers or reclaim path engineers since they can be given different orders (resulting in a clear commands being sent)
            if not(oUnit[M28Engineer.refiAssignedAction] == M28Engineer.refActionSpecialShieldDefence) and not(oUnit[M28Engineer.refiAssignedAction] == M28Engineer.refActionManageGameEnderTemplate) and not(oUnit[M28Engineer.refiAssignedAction] == M28Engineer.refActionReclaimPath) then
