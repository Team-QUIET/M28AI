...
function ManageMassStalls(iTeam)
    ...
    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains]) == false then
        ...
        if M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass] or ((GetGameTimeSeconds() >= 120 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] >= 3 and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastEnergyStall] or -100) >= 10 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageEnergyPercentStored] >= 0.99)) or (M28Team.tTeamData[iTeam][M28Team.subrefiTeamMassStored] <= 5 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] < 0.5 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and GetGameTimeSeconds() >= 80 and GetGameTimeSeconds() <= 180 and not(M28Map.bIsLowMexMap) and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 4 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyFactoryTech] >= 1 and M28Conditions.GetTeamLifetimeBuildCount(iTeam, M28UnitInfo.refCategoryEngineer) >= 2 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) then
            ...
            --Consider self-destructing underutilized naval support factories when economically stagnating
            --Trigger: no mex upgrades for 5+ minutes + have excess naval support factories
            local iTimeSinceLastNavalFacSD = GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastNavalSupportFactorySelfDestruct] or 0)
            local iTimeSinceLastMexUpgrade = GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastMexUpgrade] or 0)
            if iTimeSinceLastNavalFacSD >= 60 and iTimeSinceLastMexUpgrade >= 300 then
                --Find all T2/T3 naval support factories on the team
                local tAllNavalSupportFactories = {}
                local iNavalSupportFactoryCategory = M28UnitInfo.refCategoryNavalFactory * categories.SUPPORTFACTORY * (categories.TECH2 + categories.TECH3)
                for iBrain, oBrain in M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains] do
                    local tBrainNavalSupportFacs = oBrain:GetListOfUnits(iNavalSupportFactoryCategory, false, true)
                    if M28Utilities.IsTableEmpty(tBrainNavalSupportFacs) == false then
                        for iUnit, oUnit in tBrainNavalSupportFacs do
                            if M28UnitInfo.IsUnitValid(oUnit) then
                                table.insert(tAllNavalSupportFactories, oUnit)
                            end
                        end
                    end
                end
                local iTotalNavalSupportFacs = table.getn(tAllNavalSupportFactories)
                local iMinToKeep = math.max(2, M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])
                if bDebugMessages == true then LOG(sFunctionRef..': Checking naval factory self-destruct (stagnation), iTotalNavalSupportFacs='..iTotalNavalSupportFacs..', iMinToKeep='..iMinToKeep..', iTimeSinceLastMexUpgrade='..iTimeSinceLastMexUpgrade) end

                if iTotalNavalSupportFacs > iMinToKeep then
                    --Get main base position for distance scoring
                    local tMainBasePosition
                    for iBrain, oBrain in M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains] do
                        tMainBasePosition = oBrain.M28StartPosition or oBrain:GetStartVector3f()
                        break
                    end
                    --Score factories: prefer T2, distant, and idle factories
                    local tScoredFactories = {}
                    for iUnit, oUnit in tAllNavalSupportFactories do
                        if M28UnitInfo.IsUnitValid(oUnit) then
                            local iScore = 0
                            if EntityCategoryContains(categories.TECH2, oUnit.UnitId) then iScore = iScore + 100 end
                            iScore = iScore + M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tMainBasePosition) * 0.1
                            if oUnit:IsUnitState('Building') then iScore = iScore - 200 end
                            local oBuildTarget = oUnit:GetFocusUnit()
                            if oBuildTarget and M28UnitInfo.IsUnitValid(oBuildTarget) then
                                local iInvestedMass = (oBuildTarget:GetBlueprint().Economy.BuildCostMass or 0) * oBuildTarget:GetFractionComplete()
                                if iInvestedMass > 200 then iScore = iScore - 150 end
                            end
                            if iScore >= -100 then
                                table.insert(tScoredFactories, {oUnit = oUnit, iScore = iScore})
                            end
                        end
                    end

                    --Sort by score descending (highest score = best to destroy)
                    table.sort(tScoredFactories, function(a, b) return a.iScore > b.iScore end)

                    --Destroy factories down to minimum
                    local iFactoriesToDestroy = iTotalNavalSupportFacs - iMinToKeep
                    local iFactoriesDestroyed = 0
                    for iEntry, tFactoryData in tScoredFactories do
                        if iFactoriesDestroyed >= iFactoriesToDestroy then break end
                    ...
                    if iFactoriesDestroyed > 0 then
                        M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastNavalSupportFactorySelfDestruct] = GetGameTimeSeconds()
                    end
                end
            end
            if bDebugMessages == true then
...
function ConsiderNavalFactorySelfDestructForDominance(iTeam)
    --Self-destructs T2/T3 naval support factories when we have naval dominance but persistent low mass storage
    --This frees up mass by reclaiming underutilized factories when we're winning on water but economically constrained
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then bDebugMessages = true end
    local sFunctionRef = 'ConsiderNavalFactorySelfDestructForDominance'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)

    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains]) then
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return
    end

    --Track low mass storage duration (< 5% for 2+ minutes)
    local iCurMassStoragePercent = M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] or 0
    if iCurMassStoragePercent < 0.05 then
        if not(M28Team.tTeamData[iTeam][M28Team.refiTimeStartedLowMassStorage]) then
            M28Team.tTeamData[iTeam][M28Team.refiTimeStartedLowMassStorage] = GetGameTimeSeconds()
        end
    else
        M28Team.tTeamData[iTeam][M28Team.refiTimeStartedLowMassStorage] = nil
    end

    --Track naval dominance duration (have 50%+ more naval threat than enemy)
    local bIsAheadOnNavy = false
    if M28Utilities.IsTableEmpty(M28Map.tPondDetails) == false then
        local iEnemyNavalThreat, iAllyNavalThreat = 0, 0
            ...
            if M28Utilities.IsTableEmpty(tPondSubtable[M28Map.subrefPondWaterZones]) == false then
                for iWaterZone, tWZData in tPondSubtable[M28Map.subrefPondWaterZones] do
                    if tWZData[M28Map.subrefWZTeamData] and tWZData[M28Map.subrefWZTeamData][iTeam] then
                        ...
                        iEnemyNavalThreat = iEnemyNavalThreat + (tWZTeamData[M28Map.subrefWZThreatEnemyAntiNavy] or 0) + (tWZTeamData[M28Map.subrefWZThreatEnemySubmersible] or 0) + (tWZTeamData[M28Map.subrefWZThreatEnemyVsSurface] or 0)
                        iAllyNavalThreat = iAllyNavalThreat + (tWZTeamData[M28Map.subrefWZThreatAlliedAntiNavy] or 0) + (tWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 0) + (tWZTeamData[M28Map.subrefWZThreatAlliedSurface] or 0)
                    end
                end
            end
        end
        --Ahead on navy if we have 50%+ more threat than enemy and have at least 1000 threat
        if iAllyNavalThreat >= 1000 and iAllyNavalThreat >= iEnemyNavalThreat * 1.5 then
            bIsAheadOnNavy = true
        end
        if bDebugMessages == true then LOG(sFunctionRef..': iAllyNavalThreat='..iAllyNavalThreat..', iEnemyNavalThreat='..iEnemyNavalThreat..', bIsAheadOnNavy='..tostring(bIsAheadOnNavy)) end
    end
    if bIsAheadOnNavy then
        if not(M28Team.tTeamData[iTeam][M28Team.refiTimeStartedNavalDominance]) then
            M28Team.tTeamData[iTeam][M28Team.refiTimeStartedNavalDominance] = GetGameTimeSeconds()
        end
    else
        M28Team.tTeamData[iTeam][M28Team.refiTimeStartedNavalDominance] = nil
    end

    --Check if trigger conditions are met: ahead on navy 3+ min AND low mass 2+ min
    local iLowMassDuration = M28Team.tTeamData[iTeam][M28Team.refiTimeStartedLowMassStorage] and (GetGameTimeSeconds() - M28Team.tTeamData[iTeam][M28Team.refiTimeStartedLowMassStorage]) or 0
    local iNavalDominanceDuration = M28Team.tTeamData[iTeam][M28Team.refiTimeStartedNavalDominance] and (GetGameTimeSeconds() - M28Team.tTeamData[iTeam][M28Team.refiTimeStartedNavalDominance]) or 0
    local iTimeSinceLastNavalFacSD = GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastNavalSupportFactorySelfDestruct] or 0)

    if bDebugMessages == true then LOG(sFunctionRef..': iLowMassDuration='..iLowMassDuration..', iNavalDominanceDuration='..iNavalDominanceDuration..', iTimeSinceLastNavalFacSD='..iTimeSinceLastNavalFacSD) end

    if iTimeSinceLastNavalFacSD < 60 or iLowMassDuration < 120 or iNavalDominanceDuration < 180 then
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return
    end
    ...
    local iTotalNavalSupportFacs = table.getn(tAllNavalSupportFactories)
    local iMinToKeep = math.max(2, M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])
    if bDebugMessages == true then LOG(sFunctionRef..': iTotalNavalSupportFacs='..iTotalNavalSupportFacs..', iMinToKeep='..iMinToKeep) end
                ...
                if M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refbPrioritiseProduction] and M28Conditions.TeamHasLowMass(iTeam) and (not(tLZOrWZTeamData[M28Map.refbBaseInSafePosition]) or tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] or ((tLZOrWZTeamData[M28Map.subrefMexCountByTech][iMexTechLevel - 1] or 0) == 0)) then
                    ...
                    if iMexTechLevel >= 3 or
                            ((tLZOrWZData[M28Map.subrefLZOrWZMexCount] > 1 or tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and
                                    ((M28Map.iMapSize >= 1000 and tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] < 2) or tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] < tLZOrWZData[M28Map.subrefLZOrWZMexCount] * 0.3 or (M28Utilities.bLoudModActive and (EntityCategoryContains(M28UnitInfo.refCategoryT3Mex, oJustBuilt.UnitId) or (not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] > 0 and iMexesOnMap <= 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])))
