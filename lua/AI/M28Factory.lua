                ...
                if not (iCategoryToGet) and M28Utilities.IsTableEmpty(tLZData[M28Map.subrefLZAdjacentLandZones]) == false then
                    ...
                    for _, iAdjLZ in tLZData[M28Map.subrefLZAdjacentLandZones] do
                        if iFactoryTechLevel < 3 or not (bDontConsiderBuildingMAA) then
                            iCategoryToGet = GetLandZoneSupportCategoryWanted(oFactory, iTeam, tLZTeamData, iPlateau, iLandZone, iAdjLZ, bDontConsiderBuildingMAA, bConsiderMobileShields, bConsiderMobileStealths, bConsiderAbsolvers, (true and GetGameTimeSeconds() >= 21*60 and bDontGetCombat),nil,bDontConsiderLandScouts, bDontConsiderBuildingSkirmishers)
                        elseif M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= 1000 then
                            iCategoryToGet = GetLandZoneSupportCategoryWanted(oFactory, iTeam, tLZTeamData, iPlateau, iLandZone, iAdjLZ, false, bConsiderMobileShields, bConsiderMobileStealths, bConsiderAbsolvers,(true and GetGameTimeSeconds() >= 21*60 and bDontGetCombat),nil,bDontConsiderLandScouts, bDontConsiderBuildingSkirmishers)
                        else
                            iCategoryToGet = GetLandZoneSupportCategoryWanted(oFactory, iTeam, tLZTeamData, iPlateau, iLandZone, iAdjLZ, bDontConsiderBuildingMAA, bConsiderMobileShields, bConsiderMobileStealths, bConsiderAbsolvers,(true and GetGameTimeSeconds() >= 21*60 and bDontGetCombat),nil,bDontConsiderLandScouts, bDontConsiderBuildingSkirmishers)
                        end
