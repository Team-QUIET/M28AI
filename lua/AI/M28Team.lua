...
tTeamData = {} --[x] is the aiBrain.M28Team number - stores certain team-wide information
    ...
    subreftLandEmergencyState = 'M28TeamLandEmergency'
        subrefiLandEmergencyEnemyBrainIndex = 'LandEmEnemy'
        subreftLandEmergencyEnemyBase = 'LandEmBase'
        subrefiLandEmergencyPlateau = 'LandEmPlateau'
...
function CreateNewTeam(aiBrain)
    ...
    tTeamData[iTotalTeamCount][subreftLandEmergencyState] = {
                ...
                for iBrain, oBrain in tTeamData[iM28Team][subreftoFriendlyActiveM28Brains] do
                    ...
                    if not(tbBrainsWithActiveUpgradeByIndex[oBrain:GetArmyIndex()]) and (not(oBrain[M28Overseer.refbPrioritiseLowTech]) or tTeamData[iM28Team][refiConstructedExperimentalCount] > 0 or oBrain[M28Economy.refiOurHighestLandFactoryTech] < tTeamData[iM28Team][subrefiHighestEnemyGroundTech] - 1 or M28Utilities.IsTableEmpty(tTeamData[iM28Team][reftEnemyLandExperimentals])) and not(oBrain[M28Overseer.refbPrioritiseAir]) then
                        ...
                        if oBrain[M28Map.refbCanPathToEnemyBaseWithLand] and oBrain[M28Economy.refiOurHighestLandFactoryTech] > 0 and (oBrain[M28Economy.refiOurHighestLandFactoryTech] < tTeamData[iM28Team][subrefiHighestEnemyGroundTech] or (bNearbyUpgradedEnemyACU and oBrain[M28Economy.refiOurHighestLandFactoryTech] == 1) or (tTeamData[iM28Team][subrefiTeamGrossMass] >= 5 * (1 + (tTeamData[iM28Team][subrefiActiveM28BrainCount] - 1) * 0.5) and tTeamData[iM28Team][subrefiTeamGrossEnergy] >= 40 * (1 + (tTeamData[iM28Team][subrefiActiveM28BrainCount] - 1)*0.5)) or M28Conditions.GetTeamLifetimeBuildCount(iM28Team, (M28UnitInfo.refCategoryLandCombat * M28UnitInfo.ConvertTechLevelToCategory(tTeamData[iM28Team][subrefiHighestFriendlyFactoryTech]) + M28UnitInfo.refCategoryIndirect * M28UnitInfo.ConvertTechLevelToCategory(tTeamData[iM28Team][subrefiHighestFriendlyFactoryTech]))) >= 20 * (2-tTeamData[iM28Team][subrefiHighestFriendlyFactoryTech]) + 15 * tTeamData[iM28Team][subrefiActiveM28BrainCount]) then
                            ...
                            if bWantUpgrade and oBrain:GetEconomyStoredRatio('MASS') <= 0.2 then
                            ...
                            if bWantUpgrade and (iExistingBrainsWithHQUpgrades < tTeamData[iM28Team][subrefiActiveM28BrainCount] * 0.75 or tTeamData[iM28Team][subrefiTeamMassStored] >= 300 * tTeamData[iM28Team][subrefiActiveM28BrainCount]) then
                    ...
                    for iBrain, oBrain in tTeamData[iM28Team][subreftoFriendlyActiveM28Brains] do
                        if oBrain[M28Economy.refiOurHighestAirFactoryTech] == 2 then
                            ...
                            if bWantUpgrade then
                                ...
                                M28Economy.FindAndUpgradeUnitOfCategory(oBrain, M28UnitInfo.refCategoryAirHQ * categories.TECH2, 3)
                            end
                        end
                    end
                end
...
local function GetLandEmergencyModeSummary(tiModeByBrainIndex)
    local tsSummary = {}
    if M28Utilities.IsTableEmpty(tiModeByBrainIndex) == false then
...
local function AreLandEmergencyPositionsClose(tPositionA, tPositionB, iOptionalDistance)
    if tPositionA and tPositionB and tPositionA[1] and tPositionA[3] and tPositionB[1] and tPositionB[3] then
        return M28Utilities.GetDistanceBetweenPositions(tPositionA, tPositionB) <= (iOptionalDistance or iLandEmergencyEnemyBaseMatchDistance)
    end
    return false
end

function GetLandEmergencyState(iTeam)
    return GetOrCreateLandEmergencyState(iTeam)
end

function GetLandEmergencyModeForBrain(aiBrain)
    if aiBrain and aiBrain.M28Team and tTeamData[aiBrain.M28Team] then
        local tLandEmergencyState = GetOrCreateLandEmergencyState(aiBrain.M28Team)
        return (tLandEmergencyState[subreftiLandEmergencyModeByBrainIndex] or {})[aiBrain:GetArmyIndex()] or iLandEmergencyModeNone
    end
...
function UpdateLandEmergencyState(iTeam)
    ...
    local iCurTime = GetGameTimeSeconds()
    if iCurTime - (tTeamData[iTeam][refiTimeLastLandEmergencyRefresh] or -100) < iLandEmergencyRefreshInterval then
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return
    end
    tTeamData[iTeam][refiTimeLastLandEmergencyRefresh] = iCurTime

    local tLandEmergencyState = GetOrCreateLandEmergencyState(iTeam)
    ...
    local ttBestZoneByEnemyBrainIndex = {}
    local toRelevantEnemyBrains = tTeamData[iTeam][subreftoEnemyBrains]

    if M28Map.bMapLandSetupComplete and M28Utilities.IsTableEmpty(M28Map.tAllPlateaus) == false then
            ...
            if M28Utilities.IsTableEmpty(tPlateauData[M28Map.subrefPlateauLandZones]) == false then
                for iLandZone, tLZData in tPlateauData[M28Map.subrefPlateauLandZones] do
                    ...
                    if tLZTeamData then
                        ...
                        if tLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] or tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] or tLZTeamData[M28Map.subrefbLZWantsDFSupport] or iEnemyThreat >= 250 then
                            local iAllyThreat = tLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0
                            local iThreatDeficit = math.max(0, iEnemyThreat - iAllyThreat * 0.85)
                            local iZoneWeight = 1
                            if tLZTeamData[M28Map.subrefLZbCoreBase] then iZoneWeight = iZoneWeight + 1 end
                            if (tLZTeamData[M28Map.refiModDistancePercent] or 1) <= 0.35 then iZoneWeight = iZoneWeight + 0.5 end
                            if tLZTeamData[M28Map.subrefbLZWantsDFSupport] then iZoneWeight = iZoneWeight + 0.5 end
                            if tLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] then iZoneWeight = iZoneWeight + 0.5 end
                            iZoneWeight = iZoneWeight + math.min(1, (tLZTeamData[M28Map.subrefLZTValue] or 0) / 300)

                            local iZoneScore = iThreatDeficit * iZoneWeight
                            local tbEnemyUnitsByBrainIndex = {}
                            local bAssignedThreatToEnemyBrain = false

                            if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                for _, oEnemy in tLZTeamData[M28Map.subrefTEnemyUnits] do
                                    if M28UnitInfo.IsUnitValid(oEnemy) and oEnemy.GetAIBrain and oEnemy:GetAIBrain() then
                                        local iEnemyBrainIndex = oEnemy:GetAIBrain():GetArmyIndex()
                                        if not(tbEnemyUnitsByBrainIndex[iEnemyBrainIndex]) then tbEnemyUnitsByBrainIndex[iEnemyBrainIndex] = {} end
                                        table.insert(tbEnemyUnitsByBrainIndex[iEnemyBrainIndex], oEnemy)
                                    end
                                end
                            end

                            if M28Utilities.IsTableEmpty(tbEnemyUnitsByBrainIndex) == false then
                                for iEnemyBrainIndex, toEnemyUnits in tbEnemyUnitsByBrainIndex do
                                    local iEnemyWeightedScore = M28UnitInfo.GetCombatThreatRating(toEnemyUnits, true) * iZoneWeight
                                    if iEnemyWeightedScore > 0 then
                                        bAssignedThreatToEnemyBrain = true
                                        tiScoreByEnemyBrainIndex[iEnemyBrainIndex] = (tiScoreByEnemyBrainIndex[iEnemyBrainIndex] or 0) + iEnemyWeightedScore
                                        if not(ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex]) or iEnemyWeightedScore > ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex][subrefiLandEmergencyScore] then
                                            ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex] = {
                                                [subrefiLandEmergencyPlateau] = iPlateau,
                                                [subrefiLandEmergencyTargetLZ] = iLandZone,
                                                [subrefiLandEmergencyScore] = iEnemyWeightedScore,
                                            }
                                        end
                                    end
                                end
                            ...
                            if not(bAssignedThreatToEnemyBrain) and iZoneScore > 0 and M28Utilities.IsTableEmpty(toRelevantEnemyBrains) == false then
                                local oClosestEnemyBrain = nil
                                local iClosestEnemyBaseDistance = 100000
                                local tReferencePosition = tLZTeamData[M28Map.reftClosestEnemyBase] or tLZData[M28Map.subrefMidpoint]
                                for _, oEnemyBrain in toRelevantEnemyBrains do
                                    if not(oEnemyBrain:IsDefeated()) and not(oEnemyBrain.M28IsDefeated) and M28Map.PlayerStartPoints[oEnemyBrain:GetArmyIndex()] then
                                        local iCurDistance = M28Utilities.GetDistanceBetweenPositions(tReferencePosition, M28Map.PlayerStartPoints[oEnemyBrain:GetArmyIndex()])
                                        if iCurDistance < iClosestEnemyBaseDistance then
                                            iClosestEnemyBaseDistance = iCurDistance
                                            oClosestEnemyBrain = oEnemyBrain
                                        end
                                    end
                                end
                                if oClosestEnemyBrain then
                                    local iEnemyBrainIndex = oClosestEnemyBrain:GetArmyIndex()
                                    tiScoreByEnemyBrainIndex[iEnemyBrainIndex] = (tiScoreByEnemyBrainIndex[iEnemyBrainIndex] or 0) + iZoneScore
                                    if not(ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex]) or iZoneScore > ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex][subrefiLandEmergencyScore] then
                                        ttBestZoneByEnemyBrainIndex[iEnemyBrainIndex] = {
                                            [subrefiLandEmergencyPlateau] = iPlateau,
                                            [subrefiLandEmergencyTargetLZ] = iLandZone,
                                            [subrefiLandEmergencyScore] = iZoneScore,
                                        }
                                    end
                                end
                            end
    ...
    local iBestEnemyBrainIndex = nil
    local iBestEnemyScore = 0
    for iEnemyBrainIndex, iEnemyScore in tiScoreByEnemyBrainIndex do
        if iEnemyScore > iBestEnemyScore then
            iBestEnemyScore = iEnemyScore
            iBestEnemyBrainIndex = iEnemyBrainIndex
        end
    end

    local iLastAboveThreshold = tLandEmergencyState[subrefiLandEmergencyLastAboveThreshold] or 0
    local bShouldKeepEmergencyActive = false
    if iBestEnemyScore >= iLandEmergencyActivationScore then
        bShouldKeepEmergencyActive = true
        iLastAboveThreshold = iCurTime
    elseif bWasActive then
        if iBestEnemyScore >= iLandEmergencyClearScore then
            bShouldKeepEmergencyActive = true
            iLastAboveThreshold = iCurTime
        elseif iCurTime - iLastAboveThreshold < iLandEmergencyClearDuration then
            bShouldKeepEmergencyActive = true
        end
    end
    ...
    if bShouldKeepEmergencyActive then
        ...
        if tBestZoneForEnemy then
            iTargetPlateau = tBestZoneForEnemy[subrefiLandEmergencyPlateau]
            iTargetLZ = tBestZoneForEnemy[subrefiLandEmergencyTargetLZ]
        else
            iBestEnemyBrainIndex = iPrevEnemyBrainIndex
            iTargetPlateau = iPrevTargetPlateau
            iTargetLZ = iPrevTargetLZ
        end

        if iBestEnemyBrainIndex and M28Map.PlayerStartPoints[iBestEnemyBrainIndex] then
            tEnemyBase = {
                M28Map.PlayerStartPoints[iBestEnemyBrainIndex][1],
                M28Map.PlayerStartPoints[iBestEnemyBrainIndex][2],
                M28Map.PlayerStartPoints[iBestEnemyBrainIndex][3],
            }
        else
            tEnemyBase = tLandEmergencyState[subreftLandEmergencyEnemyBase]
        end

        if iTargetPlateau and iTargetLZ and M28Utilities.IsTableEmpty(tTeamData[iTeam][subreftoFriendlyActiveM28Brains]) == false then
            ...
            for _, oBrain in tTeamData[iTeam][subreftoFriendlyActiveM28Brains] do
                if not(oBrain.M28IsDefeated) and not(oBrain:IsDefeated()) then
                    ...
                    local tStartPosition = M28Map.GetPlayerStartPosition(oBrain)
                    ...
                    if iBrainStartPlateau == iTargetPlateau then
                        bHardMode = true
                    elseif tTargetMidpoint and iBrainIsland == iTargetIsland and iBrainIsland > 0 then
                        local iTravelDistance = M28Utilities.GetTravelDistanceBetweenPositions(tStartPosition, tTargetMidpoint, M28Map.refPathingTypeLand)
                        if not(iTravelDistance) then iTravelDistance = M28Utilities.GetDistanceBetweenPositions(tStartPosition, tTargetMidpoint) * 1.5 end
                        if iTravelDistance and iTravelDistance <= iLandEmergencyHardTravelDistance then
                            bHardMode = true
                        end
                    end
    ...
    tLandEmergencyState[subrefiLandEmergencyEnemyBrainIndex] = bShouldKeepEmergencyActive and iBestEnemyBrainIndex or nil
    tLandEmergencyState[subreftLandEmergencyEnemyBase] = bShouldKeepEmergencyActive and tEnemyBase or nil
    tLandEmergencyState[subrefiLandEmergencyPlateau] = bShouldKeepEmergencyActive and iTargetPlateau or nil
    ...
    if bShouldKeepEmergencyActive and (bStateChanged or iCurTime - (tTeamData[iTeam][refiTimeLastLandEmergencyLog] or -100) >= iLandEmergencyLogInterval) then
        tTeamData[iTeam][refiTimeLastLandEmergencyLog] = iCurTime
        LOG('LandEmergency: Team'..iTeam..
    ...
    elseif bDebugMessages == true then
        LOG(sFunctionRef..': iTeam='..iTeam..'; active='..tostring(bShouldKeepEmergencyActive)..'; enemy='..(iBestEnemyBrainIndex or 'nil')..'; target=P'..(iTargetPlateau or 'nil')..'LZ'..(iTargetLZ or 'nil')..'; score='..math.floor(iBestEnemyScore or 0))
    end
...
function TeamOverseer(iM28Team)
    while tTeamData[iM28Team][subrefiActiveM28BrainCount] > 0 do
        ...
        ForkThread(UpdateLandEmergencyState, iM28Team)
